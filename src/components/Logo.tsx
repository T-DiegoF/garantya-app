...
export function Logo({ showWordmark = true, size = 40, inverted = false, ...props }: Readonly<LogoProps>) {
  const bg   = inverted ? "#F5F0E8" : "#1C1917";
  const fg   = inverted ? "#1C1917" : "#F5F0E8";
  const text = inverted ? "#F5F0E8" : "#1C1917";

  const icon = (
    <>
      <circle cx="45" cy="45" r="40" fill={bg} />
      <rect x="23" y="40" width="44" height="28" rx="4" fill={fg} />
      <polygon points="45,12 73,40 17,40" fill={fg} />
      <line x1="29" y1="51" x2="51" y2="51" stroke={bg} strokeWidth="2" strokeLinecap="round" opacity="0.35" />
      <line x1="29" y1="60" x2="46" y2="60" stroke={bg} strokeWidth="2" strokeLinecap="round" opacity="0.35" />
  ...
  return (
    <svg width={size * 4.5} height={size} viewBox="0 0 360 90" xmlns="http://www.w3.org/2000/svg" {...props}>
      ...
      <text x="98" y="62" fontFamily="Inter, sans-serif" fontWeight="900" fontSize="46" letterSpacing="-3">
        <tspan fill={text}>Garant</tspan>
        <tspan fill="#A07850" fontStyle="italic" dx="-2">Y</tspan>
        <tspan fill={text} fontStyle="normal" dx="-1">a</tspan>
      </text>
    </svg>
  );
}
